    ...
    def generate_DPT(aif, dpt, dpt_proxy, beta=None, iter_folder=None, last_dpt=None):
        ...
        if finite_differences:
            ...
            if not adaptive_grid:
                ...
                # if k > 1:
                t0 = time.time()
                depth_maps, Z, k_min_indices, all_losses = section_search.grid_search_opt_k(aif, defocus_stack, indices=indices, min_Z=min_Z, max_Z=max_Z, num_Z=num_Z, k=k, beta=beta, proxy=dpt_proxy, gamma=gamma, similarity_penalty=similarity_penalty, last_dpt=last_dpt, gss_window=gss_window, verbose=verbose, windowed=windowed_mse, test=test)
                if verbose:
                    print('GRID SEARCH DURATION', time.time()-t0)
                # else:    
