...
def test_compute_shifted_indices_shape():
    width, height = 10, 12
    K = MAX_KERNEL_SIZE
    row_indices, col_indices = forward_model.compute_shifted_indices(width, height, MAX_KERNEL_SIZE)
...
def test_computer_all_values_above_thresh():
    ...
    dpt = np.full((width, height), 2.0, dtype=np.float32)
    r = forward_model.computer(dpt, dataset_params)
    assert np.all(r >= dataset_params.thresh)
...
def test_forward_deterministic():
    ...
    stack2 = forward_model.forward(dpt, aif, dataset_params, MAX_KERNEL_SIZE, indices=indices)
...
def test_build_fixed_pattern_csr_count():
    ...
    _, _, row, col, _ = forward_model.precompute_indices(width, height, MAX_KERNEL_SIZE)
...
def test_build_fixed_pattern_csr_equivalent_to_coo():
    ...
    template_A_stack = forward_model.build_fixed_pattern_csr(
        width, height, fs, row, col, sample_data)
    A_stack_v1 = forward_model.buildA(
        gt_dpt, u, v, row, col, mask, dataset_params, template_A_stack)
    A_stack_v2 = forward_model.buildA(
        gt_dpt, u, v, row, col, mask, dataset_params, template_A_stack=None)
    ...
    b1 = forward_model.forward(
        gt_dpt, gt_aif, dataset_params, MAX_KERNEL_SIZE,
        indices=indices, template_A_stack=template_A_stack)
    b2 = forward_model.forward(
        gt_dpt, gt_aif, dataset_params, MAX_KERNEL_SIZE,
        indices=indices, template_A_stack=None)
    assert np.allclose(b1, b2)

    o1 = section_search.objective_full(
        gt_dpt, gt_aif, defocus_stack, dataset_params, MAX_KERNEL_SIZE,
        indices=indices, template_A_stack=None)
    o2 = section_search.objective_full(
        gt_dpt, gt_aif, defocus_stack, dataset_params, MAX_KERNEL_SIZE,
        indices=indices, template_A_stack=template_A_stack)
    ...
    test_zs = [
        gt_dpt,
        rng.random(gt_dpt.shape).astype(np.float32),
        rng.random(gt_dpt.shape).astype(np.float32),
    ]

    for z in test_zs:
        v1 = section_search.objective_full(
            z, gt_aif, defocus_stack, dataset_params, MAX_KERNEL_SIZE,
            indices=indices, template_A_stack=template_A_stack)
        v2 = section_search.objective_full(
            z, gt_aif, defocus_stack, dataset_params, MAX_KERNEL_SIZE,
            indices=indices, template_A_stack=template_A_stack)
    ...
    z1, z2 = test_zs[:2]
    a1 = section_search.objective_full(
        z1, gt_aif, defocus_stack, dataset_params, MAX_KERNEL_SIZE,
        indices=indices, template_A_stack=template_A_stack)
    b1 = section_search.objective_full(
        z2, gt_aif, defocus_stack, dataset_params, MAX_KERNEL_SIZE,
        indices=indices, template_A_stack=template_A_stack)
    a2 = section_search.objective_full(
        z1, gt_aif, defocus_stack, dataset_params, MAX_KERNEL_SIZE,
        indices=indices, template_A_stack=template_A_stack)
